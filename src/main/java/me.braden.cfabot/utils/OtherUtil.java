package me.braden.cfabot.utils;

import me.braden.cfabot.CFABot;
...
{
    public final static String NEW_VERSION_AVAILABLE = "There is a new version of CFABot available!\n"
                    + "Current version: %s\n"
                    + "New Version: %s\n\n";
    ...
    public static Path getPath(String path)
    {
        ...
        if(path.toLowerCase().startsWith(WINDOWS_INVALID_PATH))
        {
            ...
            try
            {
                path = new File(CFABot.class.getProtectionDomain().getCodeSource().getLocation().toURI()).getParentFile().getPath() + File.separator + filename;
            }
    ...
    {
        if(CFABot.class.getPackage()!=null && CFABot.class.getPackage().getImplementationVersion()!=null)
            return CFABot.class.getPackage().getImplementationVersion();
        else
            return "UNKNOWN";
    }
